...
class _CreatePasswordScreenState extends ConsumerState<CreatePasswordScreen> {
  ...
  void _onSubmit() async {
    if (!_isPasswordStrong()) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Please ensure your password meets all strength requirements.'),
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      if (pendingReg == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Registration session expired. Please start over.'), backgroundColor: AppColors.error),
        );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Create Password', style: TextStyle(fontFamily: AppTypography.displayFont, fontWeight: FontWeight.w700)),
        centerTitle: true,
        automaticallyImplyLeading: false, // Prevent going back without password
      ),
      body: SafeArea(
        child: Center(
        ...
        children: [
          Semantics(
            ...
            child: Container(
              ...
              child: const Icon(Icons.vpn_key_rounded, color: AppColors.primaryRuby, size: 36),
            ),
          ),
          ...
          Text(
            'Your email has been verified successfully. Create a strong password to complete registration.',
